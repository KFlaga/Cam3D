...
namespace CamUnitTest
{
    ...
    public class Rational3ModelTests
    {
        ...
        public void Test_Distort()
        {
            ...
            model.Coeffs[0] = 1e-1; // k1
            model.Coeffs[1] = -2e-2; // k2
            model.Coeffs[2] = 5e-3; // k3
            ...
            double expectedXd = 0.5 * (1 + 0.5 * Math.Sqrt(2) * model.Coeffs[0]) /
                (1 + 0.5 * Math.Sqrt(2) * model.Coeffs[1] + 0.5 * model.Coeffs[2]);
        ...
        public void Test_Undistort()
        {
            ...
            model.Coeffs[0] = 0.1; // k1
            model.Coeffs[1] = 0.02; // k2
            model.Coeffs[2] = (0.78 * Math.Sqrt(2.0) - 2.0) / 11.0; // k3

            Vector2 realPoint = new Vector2(1, 1);

            double ru = 0.5 * Math.Sqrt(2.0);
            double distortedX = 0.5 * (1.0 + ru * model.Coeffs[0]) /
                (1.0 + ru * model.Coeffs[1] + ru * ru * model.Coeffs[2]);
