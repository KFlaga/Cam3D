...
namespace CalibrationModule
{
    public class ShapesGridCPFinder : CalibrationPointsFinder
    {
        ...
        void FillCalibrationGrid()
        {
            ...
            ComputeLocalAxes(out shapeX, out shapeY);
            ...
            FillCalibGridLine(2, 0, 0.25f, 1.0f, true);
            FillCalibGridLine(0, 2, 1.0f, 0.25f, false);

            for(int row = 1; row < CalibGrid.RowCount; ++row)
            {
                // Add all rows
                FillCalibGridLine(1, row, 0.25f, 1.0f, true);
            }
        ...
        private void ComputeLocalAxes(out CalibrationShape shapeX, out CalibrationShape shapeY)
        {
        ...
        public override void InitParameters()
        {
            ...
            ProcessorParameter treshLow = new ProcessorParameter(
                "Size Elimination Threshold Low", "SETL",
                "System.Single", 0.1f, 0.0001f, 1.0f);
            Parameters.Add(treshLow);

            ProcessorParameter treshHigh = new ProcessorParameter(
               "Size Elimination Threshold High", "SETH",
               "System.Single", 10.0f, 1.0f, 1000.0f);
            Parameters.Add(treshHigh);

            ProcessorParameter useMedianFilter = new ProcessorParameter(
               "Use Median Prefiltering", "UMF",
               "System.Boolean", false, false, true);
            Parameters.Add(useMedianFilter);

            ProcessorParameter medianFilterRadous = new ProcessorParameter(
               "Median Filter Radius (opt)", "MFR",
               "System.Int32", 3, 1, 9);
            Parameters.Add(medianFilterRadous);

            ProcessorParameter doStretch = new ProcessorParameter(
               "Stretch/Saturate histogram", "SSH",
               "System.Boolean", false, false, true);
            Parameters.Add(doStretch);

            ProcessorParameter saturateDark = new ProcessorParameter(
               "Dark saturation treshold", "DSH",
               "System.Single", 0.2f, 0.0f, 1.0f);
            Parameters.Add(saturateDark);

            ProcessorParameter saturateLight = new ProcessorParameter(
               "Light saturation treshold", "LSH",
        ...
        {
            PointSizeTresholdLow = (float)ProcessorParameter.FindValue("SETL", Parameters);
            PointSizeTresholdHigh = (float)ProcessorParameter.FindValue("SETH", Parameters);
            UseMedianFilter = (bool)ProcessorParameter.FindValue("UMF", Parameters);
            MedianFilterRadius = (int)ProcessorParameter.FindValue("MFR", Parameters);
            PerformHistogramStretch = (bool)ProcessorParameter.FindValue("SSH", Parameters);
            SaturateDarkTreshold = (float)ProcessorParameter.FindValue("DSH", Parameters);
            SaturateLightTreshold = (float)ProcessorParameter.FindValue("LSH", Parameters);
        }
