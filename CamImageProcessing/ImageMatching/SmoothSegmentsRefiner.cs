    ...
    public class SmoothSegmentsRefiner : DisparityRefinement
    {
                ...
                for(int x = 1; x < map.ColumnCount - 1; ++x)
                {
                    for(int y = 1; y < map.RowCount - 1; ++y)
                    {
                        ...
                        for(int i = 0; i < dirs.Length; ++i)
                        {
                            ...
                            if(gradDispX * gradDispX + gradDispY * gradDispY < MaxDisparityDiff * MaxDisparityDiff)
                            {
                                dispX += gradDispX;
                                dispY += gradDispY;
                                n += 1.0;
        ...
        public override void InitParameters()
        {
            ...
            AlgorithmParameter itersParam = new IntParameter(
                "Interations", "ITERS", 10, 1, 1000);
            Parameters.Add(itersParam);

            AlgorithmParameter dispDiffParam = new DoubleParameter(
                "Max disparitiy difference in segment", "DISP_DIFF", 1.1, 0.001, 100.0);
            Parameters.Add(dispDiffParam);
        ...
        public override void UpdateParameters()
        {
            ...
            MaxDisparityDiff = AlgorithmParameter.FindValue<double>("DISP_DIFF", Parameters);
