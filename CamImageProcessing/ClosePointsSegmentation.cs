...
namespace CamImageProcessing
{
    ...
    {
        public double MaxPointsDiff { get; set; }
        ...
        private void CheckAndAddToSegment_Disparity(int oldX, int oldY, int newX, int newY)
        {
            if(SegmentAssignments[newY, newX] == -1 &&
                ...
                Math.Abs(GetDisparity(oldY, oldX) - GetDisparity(newY, newX)) <= MaxPointsDiff)
            {
        ...
        {
            if(SegmentAssignments[newY, newX] == -1 &&
                Math.Abs(GetGrayValue(oldY, oldX) - GetGrayValue(newY, newX)) <= MaxPointsDiff)
            {
                SegmentAssignments[newY, newX] = _currentSegment.SegmentIndex;
                _currentSegment.Pixels.Add(new Point2D(y: newY, x: newX));
                _pointStack.Push(new Point2D(y: newY, x: newX));
        ...
        {
            if(SegmentAssignments[newY, newX] == -1 &&
                Math.Abs(GetColorValue(oldY, oldX, RGBChannel.Red) - GetColorValue(newY, newX, RGBChannel.Red)) <= MaxPointsDiff &&
                Math.Abs(GetColorValue(oldY, oldX, RGBChannel.Green) - GetColorValue(newY, newX, RGBChannel.Green)) <= MaxPointsDiff &&
                Math.Abs(GetColorValue(oldY, oldX, RGBChannel.Blue) - GetColorValue(newY, newX, RGBChannel.Blue)) <= MaxPointsDiff)
            {
                SegmentAssignments[newY, newX] = _currentSegment.SegmentIndex;
                _currentSegment.Pixels.Add(new Point2D(y: newY, x: newX));
                _pointStack.Push(new Point2D(y: newY, x: newX));
        ...
        public override void UpdateParameters()
        {
            ...
            MaxPointsDiff = AlgorithmParameter.FindValue<double>("DIFF", Parameters);
        }
