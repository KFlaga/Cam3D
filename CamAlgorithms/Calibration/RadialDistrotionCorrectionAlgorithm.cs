...
namespace CamAlgorithms.Calibration
{
    ...
    public class RadialDistrotionCorrectionAlgorithm : IParameterizable
    {
        ...
        {
            var dc = Distortion.Model.DistortionCenter;
            double scaledWidth = dc.X * Scale;
            double scaledHeight = dc.Y * Scale;
            Distortion.Model.InitialCenterEstimation = new Vector2(scaledWidth, scaledHeight);
            Distortion.Model.DistortionCenter = new Vector2(scaledWidth, scaledHeight);
        }
        ...
        protected void PrepareMinimalizationAlgorithm()
        {
            ...
            _minimalization.DistortionModel = Distortion.Model;
            Vector<double> parVec = new DenseVector(Distortion.Model.ParametersCount);
            Distortion.Model.Coeffs.CopyTo(parVec);
